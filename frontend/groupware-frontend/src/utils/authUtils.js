...
export const getAuthToken = () => {
  // 'authToken' 또는 'accessToken' 키로 토큰을 가져옴
...
// empNo 또는 empId 중 하나라도 있으면 유효
const hasUserIdentifier = userData && (userData.empNo || userData.empId);
const isValid = !!(token && hasUserIdentifier);

console.log('인증 상태 확인:', isValid ? '유효' : '무효');
if (!isValid) {
  console.log('인증 실패 이유:', {
    token: token ? '존재' : '없음',
    userData: userData ? '존재' : '없음',
...
if (token) {
  ...
  // 'authToken'과 'accessToken' 두 가지 키로 모두 저장
...
if (userData) {
  ...
  console.log('사용자 데이터 저장 완료:', userData.empName || userData.empId);
}
};
